...
func saveJson(file string) bool {
	...
	// 構造体の宣言
	var test_list []AutoGenerated // スライス
	...
	for i := 0; i <= 10; i++ {
		...
		test.ClassID = i + 1                 // 0からなので+1
		test.Name = "user" + strconv.Itoa(i) // intからstringに変換
		// スライスに一個ずつ追加している
	...
	fmt.Println(out.String()) // out.String()にデータが入っている

	// ループで出力したデータをjsonに書き込む
	os.WriteFile(file, []byte(out.String()), 0666)
	return true //書き込みに成功したらtrueを返す
}
